...
st.markdown(f"""
    ...
    /* Bottoni Cronologia (Solo testo, hover effect) */
    ...
    .main .block-container {{
        ...
        padding-bottom: 120px !important; /* Spazio per non coprire i messaggi con l'input */
    }}
    .stChatMessage {{
    ...
    .stChatMessage p {{ color: #FFFFFF !important; text-shadow: 1px 1px 3px rgba(0,0,0,0.5); }}

    /* RIMOZIONE TOTALE DELLA BARRA BIANCA IN BASSO */
    ...
    /* STILE SOLO PER IL CAMPO DI TESTO */
...
def reset_chat_and_archive():
    if st.session_state.messages:
        ...
        st.session_state.chat_sessions.insert(0, {"title": smart_title, "content": list(st.session_state.messages)})
    st.session_state.messages = []
    st.session_state.current_title = "Nuova chat"
    ...
    with st.chat_message("assistant"):
        try:
            completion = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": "Sei El loco Muñoz, anima del Savoia 1908."}] + st.session_state.messages,
                temperature=0.7
            )
