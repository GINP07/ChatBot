...
@st.cache_data # <--- Ottimizzazione: Evita di rileggere il file Word a ogni click!
...
if "GROQ_API_KEY" in st.secrets:
    ...
    st.error("Manca la chiave API 'GROQ_API_KEY' nei Secrets di Streamlit!")
...
def generate_title(user, bot):
    ...
    try:
        res = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Crea un titolo di MAX 4 parole per questa conversazione. No emoji, no virgolette."},
...
# ================= INPUT & LOGICA RISPOSTA (Senza Rerun continui) =================
if prompt := st.chat_input("Scrivi al Loco..."):
    ...
    if client:
        with st.chat_message("assistant"):
            sys_msg = f"""Sei El Loco Muñoz, ultras del Savoia 1908. Orgoglioso, verace, tifoso e diretto.
            Usa queste informazioni reali sul Savoia per rispondere se pertinenti: {KNOWLEDGE_BASE}
            Se l'informazione non è presente nella conoscenza, rispondi comunque mantenendo il tuo personaggio da ultras della curva sud del Savoia."""
            ...
            try:
                ...
                # 3. Gestione Cronologia e salvataggio
                ...
                # Ricarica una sola volta alla fine per aggiornare la sidebar con il titolo corretto
                st.rerun()
